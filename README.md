...
## tools/protoc
本项目需要用到的protobuf插件，`go run tools/protoc/install_tools.go`或者运行`go generate tools/protoc/install_tools.go`或者运行`tools/protoc/install-tools.sh`，会自动安装
...
option go_package = "github.com/hopeio/cherry/protobuf/user";
// 用户
message User {
